...
#with(tibble, plot(x=tibble$datetimeposixct, y = tibble$Sub_metering_1, type = "n", ylab = "Energy sub metering", xlab=""))
#with(tibble, lines(x=datetimeposixct , y=Sub_metering_1))
#with(tibble, lines(x=datetimeposixct , y=Sub_metering_2, col = "red"))
#with(tibble, lines(x=datetimeposixct , y=Sub_metering_3, col = "blue"))
...
with(tibble, {
    #plot1
    plot(x = tibble$datetimeposixct, y=tibble$Global_active_power,  type = "n", 
         ylab = "Global Active Power (kilowatts)", xlab = "")
    lines(x = tibble$datetimeposixct, tibble$Global_active_power)
    
    #plot2
    plot(x = tibble$datetimeposixct, y=tibble$Voltage,  type = "n", 
         ylab = "Voltage", xlab = "datetime")
    lines(x = tibble$datetimeposixct, tibble$Voltage)
    
    #plot3
    with(tibble, plot(x=tibble$datetimeposixct, y = tibble$Sub_metering_1, type = "n", 
    ...
    #plot4
    plot(x = tibble$datetimeposixct, y=tibble$Global_reactive_power,  type = "n", 
         xlab = "datetime")
    lines(x = tibble$datetimeposixct, y=tibble$Global_reactive_power)
})
